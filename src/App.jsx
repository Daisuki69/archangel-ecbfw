    ...
    <div style={{padding:STYLES.txnRowPadding}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <div style={{textAlign:"right"}}>
          ...
          <div style={{fontSize:STYLES.txnAmountSize,fontWeight:900,color:tx.positive?C.green:C.dark}}>{tx.positive?"":"-"} â±{fmt(tx.amount)}</div>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:C.bg,position:"relative", fontFamily: "'Jeko', sans-serif", fontWeight: 400}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"12px 12px 100px"}}>
        <div style={{background:C.white,borderRadius:20,padding:"20px",marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{fontSize:STYLES.balanceFontSize,fontWeight:500,letterSpacing:-1}}>{showBal?`â±${fmt(balance)}`:"â± â¢â¢â¢â¢â¢â¢â¢â¢"}</div>
        ...
        <div style={{display:"flex",gap:8}}>
          {pendingChanges.length > 0 && (
            <>
              ...
              <button onClick={onCommit} style={{padding:"6px 12px",borderRadius:10,border:"none",background:C.green,color:"white",fontWeight:800,fontSize:12,cursor:"pointer"}}>ð¾ Save to GitHub</button>
            </>
          )}
